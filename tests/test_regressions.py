...
from rlic._typing import D2, FArray1D, FArray2D, UVMode, f32

SHAPE: tuple[int, int] = (31, 33)
NX: int = SHAPE[0]
NY: int = SHAPE[1]
rng: np.random.Generator = np.random.default_rng(0)
TEXTURE: FArray2D[f32] = rng.random(SHAPE, dtype="float32")

ONE: FArray2D[f32] = np.ones_like(TEXTURE)
ZERO: FArray2D[f32] = np.zeros_like(TEXTURE)
...
U1: FArray2D[f32] = np.where(cond, -ONE, ONE)

jj: np.ndarray[D2, np.dtype[np.int64]] = np.broadcast_to(
    np.atleast_2d(np.arange(NX)).T, SHAPE
)
cond = jj < NX / 2
V1: FArray2D[f32] = np.where(cond, -ONE, ONE)

KL: int = min(NX, NY)
K0: FArray1D[f32] = np.sin(np.arange(KL) * np.pi / KL, dtype="float32")
